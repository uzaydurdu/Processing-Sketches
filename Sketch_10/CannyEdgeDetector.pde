...
public class CannyEdgeDetector {
	...
	public void setEdgesImage(PImage edgesImage) {
	...
	private void computeGradients(float kernelRadius, int kernelWidth) {
		...
		for (kwidth = 0; kwidth < kernelWidth; kwidth++) {
			...
			kernel[kwidth] = (g1 + g2 + g3) / 3f / (2f * (float) PI * kernelRadius * kernelRadius);
			diffKernel[kwidth] = g3 - g2;
		}

		int initX = kwidth - 1;
		int maxX = iwidth - (kwidth - 1);
		int initY = iwidth * (kwidth - 1);
		int maxY = iwidth * (iheight - (kwidth - 1));
		
		//perform convolution in x and y directions
		for (int x = initX; x < maxX; x++) {
			for (int y = initY; y < maxY; y += iwidth) {
				int index = x + y;
		...
		for (int x = kwidth; x < iwidth - kwidth; x++) {
			for (int y = initY; y < maxY; y += iwidth) {
				float sum = 0.0f;
				int index = x + y;
				int yOffset = iwidth;
		...
		maxX = iwidth - kwidth;
		initY = iwidth * kwidth;
		maxY = iwidth * (iheight - kwidth);
		for (int x = initX; x < maxX; x++) {
			for (int y = initY; y < maxY; y += iwidth) {
				int index = x + y;
				int indexN = index - iwidth;
				int indexS = index + iwidth;
				int indexW = index - 1;
				int indexE = index + 1;
	...
	private void follow(int x1, int y1, int i1, int threshold) {
		int x0 = x1 == 0 ? x1 : x1 - 1;
		int x2 = x1 == iwidth - 1 ? x1 : x1 + 1;
		int y0 = y1 == 0 ? y1 : y1 - 1;
		int y2 = y1 == iheight -1 ? y1 : y1 + 1;
	...
	private void readLuminance() {
           for (int i = 0; i < picsize; i++) {
		data[i] = int(brightness(sourceImage.pixels[i]));
	   }
	}
	...
	private void writeEdges(int pix[]) {
		//NOTE: There is currently no mechanism for obtaining the edge data
		//in any other format other than an INT_ARGB type BufferedImage.
		//This may be easily remedied by providing alternative accessors.
		if (edgesImage == null) {
			edgesImage = createImage(iwidth, iheight,RGB);
		}
		for (int i=0;i<iwidth*iheight;i++) edgesImage.pixels[i] = color(255-pix[i],255-pix[i],255-pix[i]);
	}
